...
for ch_bits in ch_bit_list:
	...
	for i in range(128):
		...
		# encrypt ECB again
		my_cipher_inner = cipher.encrypt(my_plain_text)
		hw_1.append(hamming_distance(my_cipher,my_cipher_inner))
		if debug:
			print(f"InnerCipher={my_cipher_inner}")

	plt.figure(0)
	plt.hist(hw_1)
	plt.title(f"Histogram for {ch_bits} as Change")
	plt.xlim(0,128)
	plt.ylim(0,128)
